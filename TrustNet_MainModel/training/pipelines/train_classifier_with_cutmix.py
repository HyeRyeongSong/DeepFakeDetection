...
from training.transforms.cutmix import cutmix_apply, MixupBCELoss
...
def main():
    ...
    # For CutMix
    loss_functions = {"classifier_loss": loss, "BCE" : MixupBCELoss()}
...
def train_epoch(current_epoch, loss_functions, model, optimizer, scheduler, train_data_loader, summary_writer, conf,
    ...
    for i, sample in pbar:
        ...
        x, index, lam = cutmix_apply(imgs, 1)
        x, index = x.cuda(), index.cuda()
        y_true1, y_true2 = labels, labels[index]
        lam = torch.from_numpy(lam).cuda().float()
        
        labels_dict = {'y_true1': y_true1, 'y_true2': y_true2, 'lam': lam}
        
        labels = lam.reshape(-1, 1) * y_true1 + (1. - lam.reshape(-1, 1)) * y_true2
        
        #out_labels = model(imgs)
        out_labels = model(x).cuda()
        ...
        # Loss Function For CutMix
        
        loss = loss_functions["BCE"](out_labels, labels_dict)
        fake_loss = 0
        real_loss = 0
        fake_idx = labels > 0.5
        real_idx = labels <= 0.5
